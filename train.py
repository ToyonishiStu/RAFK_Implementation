...
from model.loss import masked_l1_loss, freq_consistency_loss
...
def train_one_epoch(model, loader, optimizer, scaler, device, config):
    ...
    for batch in loader:
        ...
        with torch.amp.autocast(device.type, dtype=torch.float16, enabled=config.mixed_precision):
            ...
            loss = loss_l1 + config.lambda_freq * loss_freq
...
def main():
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        # Train
        losses = train_one_epoch(model, train_loader, optimizer, scaler, device, config)
